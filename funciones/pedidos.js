...
function getListaDePedidos(req, res) {
  conexion
    .query(
      "SELECT usuarios.id, usuarios.direccion, pedidos.estado, pedidos.fecha, pedidos.descripcion, pedidos.monto, pedidos.mododepago, productos.nombre, productos.ingredientes, infopedidos.id_pedido, infopedidos.id_producto FROM usuarios INNER JOIN pedidos ON pedidos.usuario_id  = usuarios.id JOIN infopedidos ON infopedidos.id_pedido = pedidos.id JOIN productos ON productos.id = infopedidos.id_producto ORDER BY id_pedido ASC",
    ...
    .then((respuesta) => {
      if (respuesta.lenght > 0) {
        ...
        res.status(404).json({
          ok: "false",
          res: "No hay ningún pedido registrado todavía",
        });
      }
    });
}
  ...
  conexion
    ...
    .then((pedido) => {
      if (pedido.length > 0) {
        ...
        res.status(404).json({
          ok: false,
          res: "No hay nigún pedido registrado con ese id",
        });
      }
    });
}
...
async function guardarPedido(req, res) {
  try {
    ...
    console.log({ armado });
    const idPedidoSimple = await guardarPedidoArmado(
      idUsuario,
      descripcionPedido,
      fechaDePedido,
      req,
    ...
    const respuestaPostPedido = await guardarInfopedidos(
      req,
      res,
      idPedidoSimple
    );
    ...
    .then((DetalleMisPedidos) => {
      if (DetalleMisPedidos > 0) {
        ...
        res.status(404).json({
          ok: false,
          res:
            "Parece que no has hecho ningún pedido por ahora, ve a /pedidos (post) para comenzar.",
        });
      }
    });
}
function deletePedidoPorId(req, res) {
  conexion
    .query("DELETE FROM pedidos WHERE id= ?", { replacements: [req.params.id] })
    .then(() => {
      res.status(204);
    });
}
function modificarEstadoPedido(req, res) {
  conexion
    ...
    .then((resultados) => {
      res
        .status(201)
        .json({ ok: true, res: "Estado de pedido actualizado", resultados });
    });
}
async function calcularMontoPedido(req) {
  const listaDeProductos = await conexion.query("SELECT * FROM productos", {
    type: conexion.QueryTypes.SELECT,
  });
  for (i = 0; i < req.body.productos.length; i++) {
    let productoEncontrado = listaDeProductos.find((producto) => producto.nombre == req.body.productos[i].nombre);
    if (productoEncontrado) {
      const { precio } = productoEncontrado;
      let precioMultiplicado = precio * req.body.productos[i].cantidad;
...
async function buscarIdUsuario(req) {
  ...
  let infoUsuario = await conexion.query(
    "SELECT * FROM usuarios WHERE nombredeusuario = ?",
    {
      replacements: [verificarToken.nombredeusuario],
      type: conexion.QueryTypes.SELECT,
    }
...
async function guardarPedidoArmado(
  idUsuario,
  descripcionPedido,
  fechaDePedido,
  req,
  ...
  const idPedido = await conexion.query(
    "INSERT INTO pedidos (usuario_id, descripcion, fecha, mododepago, estado, monto) VALUES (?,?,?,?,?,?)",
    {
      replacements: [
        idUsuario,
        descripcionPedido,
...
async function guardarInfopedidos(req, res, idPedido) {
  await req.body.productos.forEach((producto) => {
    conexion.query(
      "INSERT INTO infopedidos (id_pedido, id_producto) VALUES (?,?)",
