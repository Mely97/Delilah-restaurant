...
function getListaDeUsuarios() {
  conexion
    .query("SELECT * FROM usuarios", { type: conexion.QueryTypes.SELECT })
    .then((respuesta) => {
      if (respuesta.lenght != undefined) {
        ...
        res.status(404).json({
          ok: "false",
          res: "No hay ningún usuario registrado todavía",
        });
      }
    });
}
    ...
    .then((usuario) => {
      if (usuario > 0) {
        res.status(409).json({
          ok: false,
          err:
            "Ya existe un usuario con ese nombredeusuario, por favor intenta con uno nuevo.",
        });
      } else {
        conexion
          .query(
            "INSERT INTO usuarios (nombredeusuario, contrasena, nombre, apellido, email, telefono, direccion, is_admin) VALUES (?,?,?,?,?,?,?,?)",
            {
              replacements: [
          ...
          .then((resultados) => {
            res
              .status(200)
              .json({ ok: true, res: "Usuario creado con éxito", resultados });
          })
          .catch((error) => {
...
function logInUsuario(req, res) {
  const usrbody = req.body;
  const { nombredeusuario, contrasena } = req.body;
  const token = jwt.sign({ nombredeusuario, contrasena }, CLAVE_CIFRADO_SERVER);
...
function getUsuarioPorNombredeusuario(req, res) {
  conexion
    .query("SELECT * FROM usuarios WHERE nombredeusuario = ?", {
      replacements: [req.params.nombredeusuario],
      type: conexion.QueryTypes.SELECT,
    })
    .then((respuesta) => {
      if (respuesta.length > 0) {
        ...
        res.status(404).json({
          ok: false,
          res: "No hay ningún usuario registrado con ese nombredeusuario",
        });
      }
    });
}
function modificarUsuarioPorNombredeusuario(req, res) {
  conexion
    .query(
      "UPDATE usuarios SET nombredeusuario = ?, contrasena = ?, nombre = ?, apellido = ?,  email = ?, telefono = ?, direccion = ?, is_admin WHERE nombredeusuario = ?",
      {
        replacements: [
          req.body.nombredeusuario,
          req.body.contrasena,
    ...
    .then((resultados) => {
      res
        .status(201)
        .json({ ok: true, res: "Usuario actualizado,", resultados });
    })
    .catch((error) => {
      res.status(409).json({
        ok: false,
        res: "Algo parece estar mal en los datos ingresados",
      });
      console.log("Algo salió mal....", error);
    });
}
function deleteUsuariosPorNombredeusuario(req, res) {
  conexion
    .query("DELETE FROM usuarios WHERE nombredeusuario= ?", {
      replacements: [req.params.nombredeusuario],
    })
    .then((resultados) => {
