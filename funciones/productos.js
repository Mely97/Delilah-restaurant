...
function getListaDeProductos(req, res) {
  conexion
    .query("SELECT * FROM productos", { type: conexion.QueryTypes.SELECT })
    .then((Listado) => {
      if (Listado.lenght != 0) {
        res.json({ Listado });
      } else {
        res.status(404).json({
          ok: "false",
          res: "No hay ningún producto registrado todavía",
        });
      }
    });
}
...
function guardarProductoPorFormulario(req, res) {
  conexion
    .query(
      "INSERT INTO productos (nombre, precio, ingredientes) VALUES (?,?,?)",
      {
    ...
    .catch((err) => {
      res
        .status(409)
        .json({ ok: false, res: "Ya existe un producto con ese nombre" });
...
function modificarProducto(req, res, err) {
  conexion
    .query(
      "UPDATE productos SET nombre = ?, precio = ?, ingredientes = ? WHERE id = ?",
      {
        replacements: [
          req.body.nombre,
          req.body.precio,
    ...
    .then((resultados) => {
      res
        .status(201)
        .json({ ok: true, res: "Producto actualizado", resultados });
    })
    .catch((error) => {
      res.status(409).json({
        ok: false,
